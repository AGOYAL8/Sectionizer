...
def soft_cosine(tokens, stem=False, lemma=False):
    """
    Apply soft cosine between two paragraphs using glove embeddings and append the scores to a dataframe
    :param tokens:
    ...
    df_glsoftcos = pd.DataFrame(glsoftcosout, columns = ['Glove + Soft Cos'])
    df_glsoftcos['token 1'] = token_1
    df_glsoftcos['token 2'] = token_2

    if stem:
        df_glsoftcos.to_csv(r'glove_cosine_stem_output15.csv',
                                     index=None, header=True)
    else:
        df_glsoftcos.to_csv(r'glove_cosine_lemma_output15.csv',
                                       index=None, header=True)
